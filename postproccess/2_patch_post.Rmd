...
#~ named 2 as this is for the 2-patch method
...
climscen <- c('rcp45-Had', 'rcp45-MIROC', 'rcp45-CNRM', 'rcp45-CAN', 'rcp85-Had', 'rcp85-MIROC', 'rcp85-CNRM', 'rcp85-CAN','historic','rcp85-had_early','rcp85-had_late')
# 
# # read in the data
...
#
#
#   # add new column identifying the climate scenario   
...
#   }
#   
#   # bind each results table together
#   if (i == 1) {
#   thin_all <- res
#   } else {
...
# 
# 
# write.csv(drought, '../ShinyApp/drought.csv')
# write.csv(thin_full, "../ShinyApp/thin_full.csv")

### read in data
thin_full <- read_csv("../ShinyApp/thin_full.csv")

rcp45 <- dplyr::filter(thin_full, level == 'RCP-45')
rcp85 <- dplyr::filter(thin_full, level == 'RCP-85')

drought <- read_csv('../ShinyApp/drought.csv')
...
# met_date <- mkdate(met)
#
# write.csv(met_date, 'met.csv')

met_date <- read_csv('../ShinyApp/met.csv')
...
# graph yearly max and min temps for each model

temps <- met_date %>% 
  ungroup() %>%
  filter(climproj != 'JF', wy < 2100) %>% 
...
```{r precip, echo = F}
#graph yearly total precip for each model


#remove historic climate level data as it starts in 1950
proj_clim <- filter(met_date, climproj != 'JF')
...
ggplot(precip, aes(wy, rain, fill = rain)) +
  geom_col() +
  facet_wrap(~climproj) + 
  ...
  labs(
    x = 'water year',
    y = 'total precip (mm)',
    title = 'total yearly precipitation by model'
      )
...
plantc <- thin_full %>% 
  ...
  summarise(
    mean = mean(plantc/1000),
    sd = sd(plantc/1000)
  )

plantc_plot <- ggplot(plantc, aes(x=wyg, y=mean, fill = level)) +
  geom_line()  +
...
```{r stemc total, echo = F}
# total stem carbon

#calculate total stem C as sum of live and dead
...
```{r carbon counts, echo=F, warning=F, message=F}
...
# get mean stem carbon across models at year 99
summary <- flux %>% 
  ungroup() %>% 
  filter(wyg == 99) %>% 
...
# calculate total precip for each model
met_sum <- met_date %>% 
  group_by(climproj) %>% 
  summarise(
...
# calculate number of unique thinning v. climate scenarios so that we can index by that value
length <- length(unique(plantc_chg$thin)) * length(unique(plantc_chg$level))

delta$diff <- round(plantc_chg$plantc[plantc_chg$wyg == end] - plantc_chg$plantc[plantc_chg$wyg == start],2)/1000
delta$level <- plantc_chg$level[1:length]
delta$thin <- plantc_chg$thin[1:length]
...
# calculate discount based on above values
x <- vector()
for(i in 1:time){
...
```{r time until return, warning=F,message=F, echo = F}
# calculate target 90% level for each model
tmp <- flux %>% 
  ungroup() %>% 
...
## for each climate model and thin
res <- data.frame()
res[1:9,1] <- climscen[1:9] # get first 9 which doesnt include drought scenarios
for (i in 1:length(climscen)) {
  ...
  #  for each model and thin, ID the wyg where the mean stemc exceeds the target 90%
...
#replace all `Inf` (I.e never reaches target) with >99
...
```{r carbon by model, echo = F}

# how does carbon change by model?
...
##### investigation the large variation during the early years of the above graphs:
...
ggplot(stream85, aes(x = wy, y = streamflow, levels = thin, color = as.factor(thin))) +
  geom_line() +
  facet_wrap(~climproj) +
  labs(
    y = 'streamflow (mm)',
...
stemc_plot <- ggplot(stemc_tot, aes(x=wyg, y=mean, group = climproj, fill = climproj)) +
  ...
  geom_hline(yintercept = stemc_tot$mean[stemc_tot$thin==100 & stemc_tot$wyg == 0 & stemc_tot$climproj == 'rcp85-Had'], color = 'darkgreen', size = 1) + 
  facet_wrap(~thin) +
  labs(
    x = 'years after thin',
    y = 'stem carbon',
