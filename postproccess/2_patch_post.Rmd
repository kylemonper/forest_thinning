...
climscen <- c('rcp45-Had', 'rcp45-MIROC', 'rcp45-CNRM', 'rcp45-CAN', 'rcp85-Had', 'rcp85-MIROC', 'rcp85-CNRM', 'rcp85-CAN','rcp85-had_early','rcp85-had_late', 'historic')

# read in the data
for (i in 1:length(climscen)) {
  
  data <- read.table(sprintf('/Users/Kyle/Google Drive/JDSF/rhessys/out/JF_thin-proj-%s', climscen[i]), header = T)
...
for (i in 1:length(clim)) {
  # read in data
  tmp = read_rhessys_met(sprintf("/Users/Kyle/Google Drive/JDSF/rhessys/clim/%s", clim[i]))
  # id projection
  tmp$climproj <- clim[i]
